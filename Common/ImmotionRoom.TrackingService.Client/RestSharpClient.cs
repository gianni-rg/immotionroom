namespace ImmotionAR.ImmotionRoom.TrackingService.ControlClient
{
    ...
    public class RestSharpClient : IRestClient
    {
        ...
        {
            //Tony: without this try/catch, in Unity everything comes to a catastrophic end. It has no sense, because the catch
            //just re-throw the exception (with a different type, though), but it is what happens.
            //Anyway, it works, so I don't care :)
            //TODO GIANNI: Have a look at this
            try
            {
                ...
                httpClient.ReadWriteTimeout = HttpReadWriteTimeout * 1000;
                httpClient.Timeout = HttpReadWriteTimeout * 1000;
                return httpClient;
            }
            catch(Exception o)
            {
                throw new WebApiClientException(o.Message);
            }
